...
from engine.config import type_table
from engine.core.sfg_dumper import SFGDumper
...
# from engine.config.type_table import get_lang_init_script_name
...
class PrelimSemanticAnalysis:
    def __init__(self, engine):
        ...
        self.resolver: Resolver = engine.resolver
    ...
    def update_current_state_bit(self, bit_id: StateDefNode, frame: ComputeFrame, current_bits, new_defined_state_set: set):
        ...
        # state idstatestmt idstate kill
        # state idstatekill
    ...
    def update_out_states(self, stmt_id, frame: ComputeFrame, status: StmtStatus, old_index_ceiling, old_status_defined_states = set()):
        ...
        # defined_stateStateDefNodeout_state_bits
        ...
        # defined_symbolUNSOLVEDout_state_bits
        if defined_symbol := frame.symbol_state_space[status.defined_symbol]:
            if isinstance(defined_symbol, Symbol) and len(defined_symbol.states) == 0:
                ...
                # print("defined_symbol,UNSOLVED",defined_symbol.states)
    ...
    def group_in_states(self, stmt_id, stmt, in_symbol_indexes, frame: ComputeFrame, status):
        ...
        for symbol_id, each_symbol_in_states in symbol_id_to_state_index.items():
            # symbolin_statesstate_idfusion_statestatus.defined_states
    ...
    def generate_external_symbol_states(self, frame: ComputeFrame, stmt_id, symbol_id, used_symbol, method_summary):
        ...
        if used_symbol.name == ENGINE_INTERNAL.THIS:
            new_state.data_type = ENGINE_INTERNAL.THIS
    ...
    def generate_and_save_analysis_summary(self, frame: ComputeFrame, method_summary: MethodSummaryTemplate):
        ...
        for each_id_set in (
            {pair[0] for pair in def_use_summary.parameter_symbol_ids}, # 
        ...
        for stmt_id in util.find_cfg_last_nodes(frame.cfg):
            ...
            if stmt.operation in RETURN_STMT_OPERATION and len(status.used_symbols) != 0: # return_symbol
            ...
            available_defined_symbols = frame.symbol_bit_vector_manager.explain(current_symbol_bits) # last_stmtlast_symbol_def
            ...
            for symbol_def_node in available_defined_symbols:
                ...
                if symbol_id in current_symbol_ids: # summarysymbol
            ...
            for symbol_id in symbol_id_to_old_state_indexes:
                ...
                # latest_statesstate_idstatesstate,summarystate
            ...
            # defined_external_symbol_idsdefined_external_symbol_idsframesymbol_def_node
    ...
    def analyze_stmts(self, frame: ComputeFrame):
        while len(frame.stmt_worklist) != 0:
            ...
            # print(f"id{stmt_id}, stmt_worklist:",frame.stmt_worklist)
            ...
            if stmt_id in frame.loop_total_rounds:
                ...
                if frame.stmt_counters[stmt_id] < self.max_analysis_round:
                    frame.stmt_worklist.add(util.graph_successors(frame.cfg, stmt_id))
                    # print("cfg",list(util.graph_successors(frame.cfg, stmt_id)))
                else:
                    frame.stmt_worklist.pop()
                    continue
            ...
            # if stmt_counts % 2000 == 0:
                # print(f"{stmt_counts/2000}stmt_states")
