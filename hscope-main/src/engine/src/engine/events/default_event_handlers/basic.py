...
def preprocess_abc_loop(data: EventData):
    ...
    for line_number, line in enumerate(code.splitlines(), start=1):
        ...
        if jmp_match:
            ...
            if target_label in defined_labels:  # 
...
def adjust_python_self(obj, first_parameter_name = "", new_name = ENGINE_INTERNAL.THIS, under_class_decl = False):
    if isinstance(obj, list):
        for i, item in enumerate(obj):
            if isinstance(item, (list, dict)):
...
def add_main_func(data: EventData):
    ...
    out_data.append({
        ...
        'name': ENGINE_INTERNAL.UNIT_INIT,
            ...
            if "*" in dt or "[" in dt:
                for i in range(len(dt) - 1, -1, -1):
                    if dt[i] == '*':
                        ...
                        util.add_to_dict_with_default_list(row, "attrs", ENGINE_INTERNAL.POINTER)
                        break
                    elif dt[i] == '[':
                        row["data_type"] = dt[:i]
                        if "attrs" not in row:
