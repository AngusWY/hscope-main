...
import dataclasses
import os
from engine.common_structs import AccessPoint, State, ComputeFrame
...
def init_imported_unit(data: EventData):
    ...
    if frame.loader.is_module_dir_id(unit_id):
        ...
        # import/import_graphimportunit%unit_init
        ...
        scope_item = unit_scope.query(
            (unit_scope.scope_kind == ENGINE_SYMBOL_KIND.METHOD_KIND) &
            (unit_scope.name == ENGINE_INTERNAL.UNIT_INIT)
        )
...
def init_new_object(data: EventData):
    ...
    for each_type_state_index in in_data.type_states:
        ...
        if loader.is_method_decl(each_type_state.value):
        ...
        elif loader.is_class_decl(each_type_state.value) or each_type_state.data_type == ENGINE_INTERNAL.THIS:
            ...
            if each_type_state.data_type == ENGINE_INTERNAL.THIS:
            ...
            if methods_in_class:
                for each_method in methods_in_class:
                    if each_method.name == ENGINE_INTERNAL.CLASS_INIT:
        ...
        # new_object_statefields {method_name: {method_decl_state_indexes}}
        ...
        defined_symbol.states.add(new_object_state_index) # statestate_idstate
