...
class UnitScopeHierarchyAnalysis:
    def __init__(self, engine, loader, unit_id, unit_info, unit_gir):
        self.engine = engine
    ...
    def discover_scopes(self):
        root_scope = Scope(
            ...
            scope_id = -1,
            parent_stmt_id = -1,
            scope_kind = ENGINE_SYMBOL_KIND.UNIT_KIND
        )
        root_scope_id = 0
        ...
        for stmt_id in sorted(self.stmt_id_to_gir.keys()):
            ...
            if row.operation == "package_stmt":
                ...
                package_scope = Scope(
                    ...
                    scope_kind = ENGINE_SYMBOL_KIND.PACKAGE_STMT,
                ...
                import_scope = Scope(
                    ...
                    scope_id = scope_id,
                    parent_stmt_id= row.parent_stmt_id,
                    scope_kind = ENGINE_SYMBOL_KIND.IMPORT_STMT,
                ...
                variable_decl_scope = Scope(
                    ...
                    scope_id = scope_id,
                    parent_stmt_id = row.parent_stmt_id,
                    scope_kind = ENGINE_SYMBOL_KIND.VARIABLE_DECL,
                    name = util.read_stmt_field(row.name),
                ...
                parameter_decl_scope = Scope(
                    ...
                    scope_id = scope_id,
                    parent_stmt_id= row.parent_stmt_id,
                    scope_kind = ENGINE_SYMBOL_KIND.PARAMETER_DECL,
                    name = util.read_stmt_field(row.name),
                ...
                export_scope = Scope(
                    ...
                    scope_id = scope_id,
                    parent_stmt_id= row.parent_stmt_id,
                    scope_kind = ENGINE_SYMBOL_KIND.EXPORT_STMT,
                    source = util.read_stmt_field(row.source),
                    name = util.read_stmt_field(row.name),
                    alias = util.read_stmt_field(row.alias)
                )
                ...
                method_decl_scope = Scope(
                    ...
                    scope_id = scope_id,
                    parent_stmt_id= row.parent_stmt_id,
                    scope_kind = ENGINE_SYMBOL_KIND.METHOD_KIND,
                    name = util.read_stmt_field(row.name),
                    attrs = util.read_stmt_field(row.attrs)
                )
                ...
                for_stmt_scope = Scope(
                    ...
                    scope_id = scope_id,
                    parent_stmt_id= row.parent_stmt_id,
                    scope_kind = ENGINE_SYMBOL_KIND.FOR_KIND
                )
                ...
                with_stmt_scope = Scope(
                    ...
                    scope_id = scope_id,
                    parent_stmt_id= row.parent_stmt_id,
                    scope_kind = ENGINE_SYMBOL_KIND.WITH_KIND
                )
                ...
                class_decl_scope = Scope(
                    ...
                    scope_id = scope_id,
                    parent_stmt_id= row.parent_stmt_id,
                    scope_kind = ENGINE_SYMBOL_KIND.CLASS_KIND,
                ...
                namespace_decl_scope = Scope(
                    ...
                    scope_id = scope_id,
                    parent_stmt_id= row.parent_stmt_id,
                    scope_kind = ENGINE_SYMBOL_KIND.NAMESPACE_KIND,
                    name = util.read_stmt_field(row.name),
                )
                ...
                block_scope = Scope(
                    ...
                    scope_id = scope_id,
                    parent_stmt_id= row.parent_stmt_id,
                    scope_kind = ENGINE_SYMBOL_KIND.BLOCK_KIND,
                )
    ...
    def summarize_symbol_decls(self):
        ...
        for row in self.scope_space:
            if row.scope_kind in [
                    ENGINE_SYMBOL_KIND.IMPORT_STMT,
                    ENGINE_SYMBOL_KIND.VARIABLE_DECL,
                    ENGINE_SYMBOL_KIND.PARAMETER_DECL,
                    ENGINE_SYMBOL_KIND.CLASS_KIND,
                    ENGINE_SYMBOL_KIND.METHOD_KIND,
                    ENGINE_SYMBOL_KIND.NAMESPACE_KIND,
            ]:
                symbol_name = row.name
                if row.scope_kind == ENGINE_SYMBOL_KIND.IMPORT_STMT:
            ...
            if row.scope_kind in [
                    ENGINE_SYMBOL_KIND.CLASS_KIND,
                    ENGINE_SYMBOL_KIND.METHOD_KIND,
                    ENGINE_SYMBOL_KIND.BLOCK_KIND,
                    ENGINE_SYMBOL_KIND.NAMESPACE_KIND,
                    ENGINE_SYMBOL_KIND.FOR_KIND,
                    ENGINE_SYMBOL_KIND.WITH_KIND,
            ]:
