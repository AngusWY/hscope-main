...
from engine.basics.import_hierarchy import ImportHierarchy
...
class StmtDefUseAnalysis:
    ...
    def add_status_with_symbol_id_sync(self, stmt, status: StmtStatus, is_decl_stmt = False, is_parameter_decl_stmt = False):
        ...
        if isinstance(defined_symbol, Symbol):
            ...
            if defined_symbol_name.startswith(ENGINE_INTERNAL.VARIABLE_DECL_PREF):
            ...
            elif defined_symbol_name == ENGINE_INTERNAL.THIS:
                defined_symbol.source_unit_id = self.unit_id
            ...
            elif defined_symbol_name == ENGINE_INTERNAL.OBJECT:
                defined_symbol.source_unit_id = self.unit_id
        ...
        for used_symbol_index in status.used_symbols:
            ...
            if used_symbol.name == ENGINE_INTERNAL.THIS:
                ...
                continue
            if used_symbol.name == ENGINE_INTERNAL.OBJECT:
                used_symbol.source_unit_id = self.unit_id
                continue
            if used_symbol.name.startswith(ENGINE_INTERNAL.VARIABLE_DECL_PREF):
                used_symbol.source_unit_id = self.unit_id
    ...
    def create_state_and_add_space(
        ...
        if data_type == ENGINE_INTERNAL.STRING:
    ...
    def require_stmt_def_use(self, stmt_id, stmt):
        ...
        defined_symbol = self.create_symbol_or_state_and_add_space(
            stmt_id, stmt.target, ENGINE_INTERNAL.REQUIRED_MODULE
        )
    ...
    def case_stmt_def_use(self, stmt_id, stmt):
        ...
        if util.is_available(stmt.name):
            defined_symbol = self.create_symbol_or_state_and_add_space(
                stmt_id, stmt.name, ENGINE_INTERNAL.CASE_AS
            )
            is_decl = True
    ...
    def namespace_decl_def_use(self, stmt_id, stmt):
        defined_symbol = self.create_symbol_or_state_and_add_space(
            stmt_id, stmt.name, ENGINE_INTERNAL.NAMESPACE_DECL
        )
    ...
    def class_decl_def_use(self, stmt_id, stmt):
        defined_symbol = self.create_symbol_or_state_and_add_space(
            stmt_id, stmt.name, ENGINE_INTERNAL.CLASS_DECL
        )
        self.add_status_with_symbol_id_sync(
            stmt,
            StmtStatus(
    ...
    def method_decl_def_use(self, stmt_id, stmt):
        # self.empty_def_use(stmt_id, stmt)
        defined_symbol = self.create_symbol_or_state_and_add_space(
            stmt_id, stmt.name, ENGINE_INTERNAL.METHOD_DECL
        )
        self.add_status_with_symbol_id_sync(
            stmt,
            StmtStatus(
    ...
    def new_object_def_use(self, stmt_id, stmt):
        ...
        if not (type_symbol and isinstance(type_symbol, Symbol)):
            return
        if self.loader.is_method_decl(type_symbol.symbol_id): # java
    ...
    def array_insert_def_use(self, stmt_id, stmt):
        ...
        defined_symbol = self.create_symbol_or_state_and_add_space(
            stmt_id, stmt.array, default_data_type = ENGINE_INTERNAL.ARRAY
        )
    ...
    def slice_read_def_use(self, stmt_id, stmt):
        ...
        for symbol in [stmt.array, stmt.start, stmt.end, stmt.step]:
            if not util.isna(symbol):
                ...
                used_symbol_list.append(-1)
        defined_symbol = self.create_symbol_or_state_and_add_space(stmt_id, stmt.target, ENGINE_INTERNAL.ARRAY)
        self.add_status_with_symbol_id_sync(
            stmt,
            StmtStatus(
                stmt_id,
