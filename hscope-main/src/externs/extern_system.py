...
import yaml
import dataclasses
import engine.apps.event_return as er
from engine.apps.app_template import EventData
...
from engine.externs.extern_rule import Rule
from engine.externs.modeling import js_api
from engine.semantic.semantic_structs import AccessPoint, InterruptionData, State, Symbol
...
class ExternSystem:
    ...
    def find_proper_rules(self, data):
        ...
        for unsolved_state_index in unsolved_callee_states:
            ...
            if len(access_path) > 0:
                ...
                # if last_point_name == "initialRender":
    ...
    def register_model_methods(self):
        ...
        if hasattr(self.options, "extern_path") and self.options.extern_path:
            all_modelings["abc"] = []
            sys.path.append(self.options.extern_path)  # 
