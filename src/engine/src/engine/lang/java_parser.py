...
class Parser(common_parser.Parser):
    ...
    def class_body(self, node, gir_node):
        ...
        for st in subtypes:
            ...
            #                 tmp_stmt["method_decl"]["parameter_decls"].insert(0, {"parameter_decl": {"name": "this", "attrs": [EngineInternal.CURRENT_INSTANCE]}})
            #             else:
            #                 tmp_stmt["method_decl"]["parameter_decls"] = [{"parameter_decl": {"name": "this", "attrs": [EngineInternal.CURRENT_INSTANCE]}}]
    ...
    def arg_list(self, node, statements):
        child = self.find_child_by_type(node, "modifiers")
        modifiers = self.read_node_text(child).split()
        modifiers.append(ENGINE_INTERNAL.PACKED_POSITIONAL_PARAMETER)
