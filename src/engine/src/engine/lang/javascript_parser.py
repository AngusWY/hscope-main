...
class Parser(common_parser.Parser):
    ...
    # {}Objectlet obj = { name: "Alice"}
    def new_object(self, node: Node, statements: list):
        # Object
        tmp_var = self.tmp_variable()
        self.append_stmts(statements, node, {"new_object": {"data_type": ENGINE_INTERNAL.OBJECT, "target": tmp_var}})
        ...
        for child in node.named_children:
            ...
            if child.type == "pair":
                ...
                """
                    , sayHello, :
                        sayHelloobjfield, fieldvaluesayHello
                    const obj = {
                        sayHello(){
    ...
    def return_statement(self, node: Node, statements: list):
        shadow_name = "undefined"   # returnundefined
        if node.named_child_count > 0:
    ...
    def while_statement(self, node: Node, statements: list):
        ...
        new_while_body.extend(new_condition_init)   # (i++ < 5)
    ...
    def with_statement(self, node: Node, statements: list):
        ...
        # with_initJavaScriptwith_stmtnamefield
    ...
    def import_statement(self, node: Node, statements: list):
        # import_stmtimport_as_stmtsourcefield
        ...
        for import_clause_child in import_clause.named_children:
            ...
            # namespace import, : import * as alias from "module-name";
            ...
            # named import, import { name [as alias], ... } from "module-name";
            else:
    ...
    # source_strNoneexport {...}Noneexport {...} from ...
    def parse_export_clause(self, node, statements, source_str=None):
        ...
        if len(export_specifiers) == 0:
            # {}name""
            # source_strside effect import
            if source_str:
                self.append_stmts(statements, node, {"import_stmt": {"name": "", "module_path": source_str, "attrs": ['init']}})
        else:
            for specifier in export_specifiers:     # specifiername [as alias]
                name = self.find_child_by_field(specifier, "name")
                shadow_name = self.parse(name, statements)
    ...
    def export_statement(self, node: Node, statements: list):
        ...
        if export_source:   # from
            ...
            if namespace_export:
                # export * as ... from ...
                alias = namespace_export.named_children[-1]    # -1comment
                shadow_alias = self.parse(alias, statements)
            ...
            if name.type == "identifier":
                ...
                for p in name.named_children:
                    ...
                    if p.type == "shorthand_property_identifier_pattern":
                        ...
                        #  const {name: n, age: a} = {name: "tom", age: 18}
                        left_child = self.find_child_by_field(p, "key")
                        right_child = self.find_child_by_field(p, "value")
    ...
    def function_declaration(self, node: Node, statements: list, from_class_body = False):
        ...
        if "*" in shadow_children:
            gir_node["attrs"].append(ENGINE_INTERNAL.GENERATOR_DECL)
        ...
        if not name:    # function_expressionarrow_function
        ...
        if simple_param:    # arrow_function, "()"
        ...
        if (node.type == "function_declaration"
            ...
            return [shadow_name]    # export_statement
        else:
            return shadow_name
    ...
    def formal_parameter(self, node: Node, statements: list):
        ...
        elif node.type == "rest_pattern":   # (...arg)
            attrs.append(ENGINE_INTERNAL.PACKED_POSITIONAL_PARAMETER)
    ...
    def class_body(self, node, gir_node):
        ...
        if init_class_method_body:
            gir_node["methods"].insert(0,
                {
                    "method_decl":{
                        "name": ENGINE_INTERNAL.CLASS_INIT,
        ...
        if static_init_class_method_body:
            gir_node["methods"].insert(0,
                {
                    "method_decl":{
                        "name": ENGINE_INTERNAL.CLASS_STATIC_INIT,
