...
from engine.core.global_stmt_states import GlobalStmtStates
from networkx.generators.classic import complete_graph


class GlobalSemanticAnalysis(PrelimSemanticAnalysis):
    def __init__(self, engine, analyzed_method_list):
    ...
    def collect_external_symbol_states(self, frame: ComputeFrame, stmt_id, stmt, symbol_id, summary: MethodSummaryTemplate, old_key_state_indexes: set):
        ...
        for old_key_state_index in old_key_state_indexes:
            ...
            if old_key_state.data_type in (ENGINE_INTERNAL.METHOD_DECL, ENGINE_INTERNAL.CLASS_DECL):
            ...
            # TODO JAVA CASE java call this()
            if stmt.operation in CALL_OPERATION and old_key_state.data_type == ENGINE_INTERNAL.THIS:
                continue
    ...
    def analyze_frame_stack(self, frame_stack: ComputeFrameStack, global_space, sfg: StateFlowGraph):
        while len(frame_stack) >= 2:
            ...
            if not self.options.quiet:
                stmt = self.loader.get_stmt_gir(frame.method_id)
                print(f"Analyzing <method {frame.method_id}> name: {frame.method_name} in {stmt.start_row}")
