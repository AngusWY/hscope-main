...
from engine.util.data_model import DataModel
from engine.config import schema
...
class ModuleSymbolsLoader:
    ...
    def _do_cache(self):
        ...
        for row in self.module_symbol_table:
            ...
            if row.symbol_type == ENGINE_SYMBOL_KIND.UNIT_SYMBOL:
    ...
    def get_all_unit_info(self):
        ...
        all_units = self.module_symbol_table.query(self.module_symbol_table.symbol_type == ENGINE_SYMBOL_KIND.UNIT_SYMBOL)
        if len(all_units) == 0:
            return []
...
class MethodLevelAnalysisResultLoader(GeneralLoader):
    ...
    def export_indexing(self):
        from engine.common_structs import CallSite
        results = []
        for (key, value) in self.item_id_to_bundle_id.items():
...
class ImportGraphLoader:
    ...
    def get_edges_and_nodes_with_edge_attrs(self, node_id, attr_dict: dict):
        """attrreal_name, weight(edge_kind), site(import_stmt_id), symbol_type()"""
...
class Loader:
    ...
    def _convert_id_to_scope(self, scope_id):
        ...
        if unit_id < 0: return None # importfakesymbol_id
    ...
    def get_method_of_class_with_name(self, class_name:str, method_name:str) -> set[int]:
        """
            method_nameclass_nameclassmethod_id
            ** of_classclass
        """
        res = set()
