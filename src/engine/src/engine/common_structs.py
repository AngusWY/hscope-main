...
from engine.util import readable_gir
...
class Scope(BasicElement):
    ...
    scope_kind: ENGINE_SYMBOL_KIND = ENGINE_SYMBOL_KIND.METHOD_KIND
...
class ParameterMapping:
    ...
    parameter_type: int = ENGINE_INTERNAL.PARAMETER_DECL
...
class SourceSymbolScopeInfo:
    source_unit_id:int = -1
    source_symbol_id:int = -1 # source_decl_stmt_id
    decl_scope_id: int = -1
    current_symbol_id: int = -1

    def __post_init__(self):
        # current_symbol_idsource_symbol_id
...
class MethodSummaryTemplate:
    ...
    raw_to_new_index: dict[int, int] = dataclasses.field(default_factory=dict) # spacecompact space
    index_to_default_value: dict[int, int] = dataclasses.field(default_factory=dict) # index -> default_value_symbol_id 
...
@dataclasses.dataclass(frozen=True)  # frozen=True 
class CallPath:
    ...
    #  __hash__  __eq__ dataclass 

    def to_tuple(self):
        return self.path  #  tuple 
