...
import engine.util.data_model as dm
...
from engine.basics.scope_hierarchy import UnitScopeHierarchyAnalysis
from engine.basics.import_hierarchy import ImportHierarchy
from engine.basics.type_hierarchy import TypeHierarchy

from engine.basics.entry_points import EntryPointGenerator
from engine.basics.control_flow import ControlFlowAnalysis
from engine.basics.stmt_def_use_analysis import StmtDefUseAnalysis
...
class BasicSemanticAnalysis:
    def __init__(self, engine):
        ...
        self.extern_system = engine.extern_system
        self.loader:Loader = engine.loader
        self.resolver: Resolver = engine.resolver
        self.entry_points = EntryPointGenerator(engine.options, engine.event_manager, engine.loader)
    ...
    def run(self):
        ...
        for unit_info in self.loader.get_all_unit_info():
            ...
            if self.options.incremental:
                ...
                if previous_scope_analysis_pack:
                    ...
                    unit_scope = UnitScopeHierarchyAnalysis(self.engine, self.loader, unit_id, unit_info, unit_gir).reuse_analysis(previous_scope_analysis_pack)

            if not unit_is_analyzed:
                unit_scope = UnitScopeHierarchyAnalysis(self.engine, self.loader, unit_id, unit_info, unit_gir).analyze()
        ...
        unit_list.reverse()
        import_analysis = ImportHierarchy(self.engine, self.loader, self.resolver, unit_list).run()
