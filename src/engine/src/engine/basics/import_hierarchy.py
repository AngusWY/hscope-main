...
class ImportHierarchy:
    def __init__(self, engine, loader, resolver, unit_list):
        self.engine = engine
    ...
    def search_public_symbols_from_scope_hierarchy(self, scope_hierarchy):
        ...
        while worklist:
            ...
            for row in scope_results:
                ...
                if row.scope_kind == ENGINE_SYMBOL_KIND.METHOD_KIND:
                    if scope_id in public_scopes:
                ...
                internal_symbols.append(row)
                if row.scope_kind in (ENGINE_SYMBOL_KIND.CLASS_KIND, ENGINE_SYMBOL_KIND.NAMESPACE_KIND):
                    if row.scope_kind == ENGINE_SYMBOL_KIND.NAMESPACE_KIND:
        ...
        while import_path_list and initial_worklist:
            ...
            if name_to_be_matched == "*":
                ...
                for each_node in initial_worklist:
                    if each_node.symbol_type == ENGINE_SYMBOL_KIND.UNIT_SYMBOL:
            ...
            for candidate_node in initial_worklist:
                if candidate_node.symbol_name == name_to_be_matched:
                    if candidate_node.symbol_type == ENGINE_SYMBOL_KIND.UNIT_SYMBOL:
    ...
    def analyze_import_stmt(self, unit_id, unit_info, stmt, external_symbols = []):
        ...
        fake_node = self.add_import_graph_node(
            symbol_type=ENGINE_SYMBOL_KIND.UNKNOWN_KIND,
    ...
    def analyze_unit_import_stmts(self, unit_id):
        ...
        import_stmts = scope_hierarchy.query(
            ...
            (scope_hierarchy.scope_kind == ENGINE_SYMBOL_KIND.IMPORT_STMT)
        )
        results = []
    ...
    def run(self):
        # import_graphnodeid node info -> import_graph_nodes
